<template>
  <div>
    <x-header :right-options="{showMore: true}" @on-click-more="{showMore: true}">天气预报-vux版</x-header>
    <group>
    ...
    <group v-for="(item,index) in weather" style="align-middle">
      {{item,index}}&nbps;{{item,fengli}}&nbps;{{item,type}}&nbps;{{item,low}}&nbps;{{item,high}}
    </group>
